...
import { checkIcon, glowIcon, plansMeta } from "@/utils/constants";
...
export default function Package({ submitedAdID }) {
    ...
    try {
      const payload =
        ...
        selectedType === "view_auction"
          ...
          : { ad: submitedAdID, subscription_plan: selectedPlan };
      ...
      if (payment?.url) {
        setStatusMessage("در حال انتقال به درگاه پرداخت...");
        setTimeout(() => {
          window.location.href = payment.url;
        }, 1500);
      } else {
        setErrorMessage("خطا در انتقال به درگاه پرداخت");
      }
    } catch (error) {
      if (error.status === 400) {
        ...
        setErrorMessage("مشکلی پیش آمده. لطفاً مجدداً تلاش کنید.");
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="px-2 md:px-0">
      {errorMessage && (
        <div className="text-error font-vazir text-sm text-center my-2">
          {errorMessage}
        </div>
      )}

      {statusMessage && (
        <div className="text-gray-500 font-vazir text-sm text-center my-2">
          {statusMessage}
        </div>
      )}
      <div
